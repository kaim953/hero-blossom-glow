  ...
  return (
    <span className={`group inline-flex items-center gap-[10px] h-[35px] select-none ${className}`}>
      ...
      <span className="relative flex-shrink-0 w-[32px] h-[32px] flex items-center justify-center">
        {/* Accent halo */}
        <span className="absolute inset-0 rounded-[11px] bg-theme-main rotate-6 opacity-70 transition-all duration-500 ease-in-out group-hover:rotate-[18deg] group-hover:opacity-100" />
        {/* Core tile */}
      ...
      {/* Wordmark */}
      <span className="flex items-baseline leading-none font-heading text-[22px] font-bold tracking-[-0.05em] text-neutral-12 transition-colors duration-500">
        Grovia
        <span className="ml-[3px] w-[5px] h-[5px] rounded-full bg-theme-main transition-transform duration-500 ease-in-out group-hover:scale-150" />
      </span>
    </span>
  );
};
